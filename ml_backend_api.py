...
class MLModels:
    ...
    def __init__(self):
        ...
        try:
            # Trust the repository automatically
            torch.hub.set_dir('./models/torch_hub')
            self.vehicle_detector = torch.hub.load('ultralytics/yolov5', 'yolov5s', force_reload=False, trust_repo=True)
            self.vehicle_detector.conf = 0.5
            logger.info("✅ Vehicle Detector (YOLOv5) loaded")
...
async def detect_license_plate(request: OCRRequest):
    ...
    try:
        ...
        # Run OCR
        ocr_results = models.ocr_reader.readtext(image)
