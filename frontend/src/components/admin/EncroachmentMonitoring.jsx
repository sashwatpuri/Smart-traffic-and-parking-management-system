...
export default function EncroachmentMonitoring() {
  ...
  const fetchEncroachments = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/encroachments', {
        headers: {
  ...
  const handleResolve = async (id) => {
    try {
      await fetch(`http://localhost:3001/api/encroachments/${id}/resolve`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const handleIgnore = async (id) => {
    try {
      await fetch(`http://localhost:3001/api/encroachments/${id}/ignore`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
